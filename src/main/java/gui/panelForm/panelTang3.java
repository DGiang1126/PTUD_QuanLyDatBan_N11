...
public class panelTang3 extends JPanel {
	...
	private void refreshBanList() {
	    panel_Ban.removeAll();
	    Ban_DAO = new DAO_Ban();
	    java.util.List<Ban> dsBan = Ban_DAO.getAllBansinTang3();
	    ...
	    int spacingY = 50; // Khoảng cách dọc giữa hai bàn

	    // Tính toán để căn giữa theo chiều dọc và ngang
	    int totalHeight = 2 * banHeight + spacingY; // Tổng chiều cao của 2 bàn + khoảng cách
	    int startX = (panelWidth - banWidth) / 2; // Căn giữa theo chiều ngang
	    int startY = (panelHeight - totalHeight) / 2; // Căn giữa theo chiều dọc

	    int x = startX;
	    int y = startY;
	    int count = 0;

	    // Chỉ lấy tối đa 2 bàn từ dsBan
	    for (Ban ban : dsBan) {
	        if (count >= 2) break; // Thoát khi đã thêm đủ 2 bàn
	        ...
	        count++;
	        y += banHeight + spacingY; // Chuyển xuống vị trí bàn tiếp theo
	    }
	...
	private RoundedPanelWithShadow createBanPanel(String tenBan, String maBan, String status, int x, int y) {
	    ...
	    // Hình ảnh của bàn
	    ImageIcon originalIcon = new ImageIcon(panelTang3.class.getResource("/img/tableTang3.jpg"));
		        ...
		        if (status.equals("ĐANG SỬ DỤNG") || status.equals("ĐÃ ĐẶT")) {
		            try {
		                ...
		                if (maBanChinh != null) { // Nếu bàn đã được gộp
		                    ...
		                    JOptionPane.showMessageDialog(panelTang3.this, tenBan + " đã được gộp với " + banChinh.getTenBan() + "!");
		                    ...
		                    if (pdbChinh != null) {
		                        ...
		                        if (kh != null && ctpdb != null) {
		                            ...
		                            JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy thông tin khách hàng của bàn chính!");
		                            panel_ChiTietBan.setVisible(false);
		                        }
		                    } else {
		                        JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy phiếu đặt bàn của bàn chính!");
		                        panel_ChiTietBan.setVisible(false);
		                    }
		                    ...
		                    if (pdb != null) {
		                        ...
		                        if (kh != null && ctpdb != null) {
		                            ...
		                            JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy thông tin khách hàng!");
		                            displayBanDetails(tenBan, status, null, null, null, null, null);
		                            panel_ChiTietBan.setVisible(true);
		                        }
		                    } else {
		                        JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy phiếu đặt bàn!");
		                        displayBanDetails(tenBan, status, null, null, null, null, null);
		                        panel_ChiTietBan.setVisible(true);
		                    }
		                }
		                ...
		                ex.printStackTrace();
		                JOptionPane.showMessageDialog(panelTang3.this, "Lỗi khi lấy thông tin: " + ex.getMessage());
		            } catch (Exception e1) {
		                e1.printStackTrace();
		            }
		        } else {
		            panel_ChiTietBan.setVisible(false);
		            JDialog dialog = new JDialog(SwingUtilities.getWindowAncestor(panelTang3.this), "ĐẶT BÀN",
		                    Dialog.ModalityType.APPLICATION_MODAL);
		            panelPhieuDatBanTang3 panelPDB = new panelPhieuDatBanTang3();
	...
	private void displayBanDetails(String tenBan, String status, String maKH, String tenKH, String sdt, String gioiTinh,
		...
		btnHuyBan.addActionListener(e -> {
			if (status.equals("ĐÃ ĐẶT")) {
				int confirm = JOptionPane.showConfirmDialog(panelTang3.this,
				...
				if (confirm == JOptionPane.YES_OPTION) {
					try {
						...
						panel_ChiTietBan.setVisible(false);
						refreshBanList();
						JOptionPane.showMessageDialog(panelTang3.this, "Hủy bàn thành công");
						panel_ChiTietBan.setVisible(false);
					} catch (SQLException ex) {
						ex.printStackTrace();
						JOptionPane.showMessageDialog(panelTang3.this, "Lỗi khi hủy bàn: " + ex.getMessage());
					}
				}
			}
		});
		...
		btnGoiMon.addActionListener(e -> {
			try {
				...
				ex.printStackTrace();
				JOptionPane.showMessageDialog(panelTang3.this, "Lỗi khi hiển thị menu: " + ex.getMessage());
			}
		});
		panel.add(btnGoiMon);
		...
		btnThanhToan.addActionListener(e -> {
			try {
				...
				if (tienCanTraText == null || tienCanTraText.isEmpty() || Integer.parseInt(tienCanTraText) <= 0) {
					JOptionPane.showMessageDialog(panelTang3.this, "Số tiền cần thanh toán không hợp lệ!");
					return;
				}
				...
				String maHD = hoaDon_DAO.getMaHDByMaKHAndStatus(maKH, "Chưa thanh toán");
				if (maHD == null) {
					JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy hóa đơn!");
					return;
				}

				showPaymentMethodDialog(tenBan, maKH, maHD, amount);
			} catch (NumberFormatException ex) {
				JOptionPane.showMessageDialog(panelTang3.this, "Số tiền không hợp lệ!");
			}
		});
		...
		try {
			...
			e1.printStackTrace();
			JOptionPane.showMessageDialog(panelTang3.this, "Lỗi khi lấy khuyến mãi: " + e1.getMessage());
		}
...
//		// Set dialog icon
		...
		for (MonAn monAn : filteredMonAn) {
			...
			btnSelect.addActionListener(e -> {
				try {
					...
					if (quantity <= 0) {
						JOptionPane.showMessageDialog(panelTang3.this, "Số lượng phải lớn hơn 0!");
						return;
					}
					...
					String maBan = Ban_DAO.getMaBanByTenBan(tenBan);
					if (maBan == null) {
						JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy mã bàn!");
						return;
					}

					String maKH = getMaKHFromPhieuDatBan(maBan);
					if (maKH == null) {
						JOptionPane.showMessageDialog(panelTang3.this, "Không tìm thấy khách hàng cho bàn này!");
						return;
					}
					...
					if (!isAddedCTHD) {
						JOptionPane.showMessageDialog(panelTang3.this, "Không thể thêm món vào hóa đơn!");
						return;
					}
					...
					updateTotal();
					JOptionPane.showMessageDialog(panelTang3.this,
							"Đã thêm " + quantity + " " + monAn.getTenMonAn() + " vào danh sách!");
				} catch (NumberFormatException ex) {
					JOptionPane.showMessageDialog(panelTang3.this, "Vui lòng nhập số lượng hợp lệ!");
				} catch (SQLException ex) {
					ex.printStackTrace();
					JOptionPane.showMessageDialog(panelTang3.this, "Lỗi cơ sở dữ liệu: " + ex.getMessage());
				}
			});
	...
	private void showMergeTableDialog(String mainTableName, String maKH) throws SQLException {
	    ...
	    try {
	        ...
	        List<Ban> availableTables = Ban_DAO.getAllBansinTang3().stream()
	    ...
	    dialog.getContentPane().add(btnMerge);
    ...
    private void showChuyenBanDialog(String tenBan, String maKH, String status) {
        ...
        for (Ban ban : banTrongList) {
            ...
            ImageIcon originalIcon = new ImageIcon(panelTang3.class.getResource("/img/dining-table.png"));
	...
	private static class ButtonEditor extends DefaultCellEditor {
		...
		private JTable table;
		private panelTang3 panel;
		private String tenBan;
		private int row;

		public ButtonEditor(JCheckBox checkBox, JTable table, String tenBan, panelTang3 panel) {
			super(checkBox);
			this.table = table;
			this.tenBan = tenBan;
			this.panel = panel;
